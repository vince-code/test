function [] = GenQueries2(fileID,Line,Bus,F_branch,F_bus,CB_settings,LinesCBs,Bus_connections)
% [] = GenQueries2(fileID,Line,Bus,F_branch,F_bus,CB_settings,LinesCBs,Bus_connections)
% Function GenQueries2 generates the queries required to check for
% selectivity one query per fault location.
...
CB_Bus = [];% Vector where the first colomn has the CB ID and the second colomn has the Bus ID
        ...
        for pp = 1:length(y)
            ...
            if ismember(faulted_CB_ID,y)
                if ~ismember(Upstream_CB_ID,ConnectedCBs)
                    if Upstream_CB_ID ~= faulted_CB_ID
                        fprintf(fileID,'  <query>\n');
                        fprintf(fileID,'    <formula>E&lt;&gt;(!CB%i.Open &amp;&amp; CB%i.Open &amp;&amp; FB%i)</formula>\n',faulted_CB_ID,Upstream_CB_ID,Bus(F_bus(k)));
                        fprintf(fileID,'    <comment></comment>\n');
                        fprintf(fileID,'  </query>\n');
                    end
                end
