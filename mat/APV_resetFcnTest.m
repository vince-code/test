function [InitialObs,Info] = APV_resetFcnTest(Info_cell)
%% ResetFcn
...
[~, nameOnly, ~] = fileparts(Info.jsonFileName);
splitResult = struct();
splitResult.div = div;
splitResult.LineIDs = [Info.Line];
splitResult.BusIDs = [Info.Bus];
splitResult.F_branch_matrix = F_branch_matrix;
splitResult.F_bus_matrix = F_bus_matrix;

splitPath = fullfile(SPLIT_FOLDER, [nameOnly, '_split.json']);
fid = fopen(splitPath, 'w');
fwrite(fid, jsonencode(splitResult), 'char');
fclose(fid);
%fprintf('Saved split JSON: %s\n', splitPath);
...
misconfiguredCBs = [];
SPLIT_DIR = [SPLIT_XML_FOLDER,'/',nameOnly];
...
for i = 1:div
    ...
    XMLFILENAME{i,1} = fullfile(SPLIT_DIR, [nameOnly, '_split_' char(string(i)) '.xml']);
    ...
    cmdVerifyTA{i,1} = strcat(VTA_FOLDER," -t0 ",XMLFILENAME{i,1});
    ...
    while WrongCBs == true
        ...
        if isempty(selectivityIdx)
            ...
            misconfiguredCBs = union(misconfiguredCBs,AllStandByCBs);
...
t_execution = toc(tstart);

result = struct();
result.executionTime = t_execution;
result.misconfiguredCBs = misconfiguredCBs;

[~, nameOnly, ~] = fileparts(Info.jsonFileName);

resultFileName = fullfile(RESULT_FOLDER, [nameOnly, '_result.json']);
fid = fopen(resultFileName, 'w');
fwrite(fid, jsonencode(result), 'char');
fclose(fid);

fprintf('\t(%.2f seconds) ', t_execution);

if ~isempty(misconfiguredCBs)
    fprintf('misconfiguredCBs: [ ');
    fprintf('%d ', misconfiguredCBs);
    fprintf(']');
else
    fprintf('OK');
end
