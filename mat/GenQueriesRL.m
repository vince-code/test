function [] = GenQueriesRL(fileID,Line,Bus,F_branch,F_bus,CB_settings,LinesCBs,Bus_connections,WrongCB_IDs)
...
Index = faulted_CB_ID == WrongCB_IDs(:,1);
Upstream_CB_ID = x(1);

if Upstream_CB_ID == faulted_CB_ID || (nnz(Index)>0 && ismember(Upstream_CB_ID,WrongCB_IDs(Index,2)))
    Upstream_CB_ID = x(2);
    fprintf(fileID,'(!CB%i.Open &amp;&amp; CB%i.Open &amp;&amp; F%i)',faulted_CB_ID,Upstream_CB_ID,Line(F_branch(1)));
    for kk = 3:length(x)
        Upstream_CB_ID = x(kk);
        if Upstream_CB_ID ~= faulted_CB_ID
            if ~(nnz(Index)>0 && ismember(Upstream_CB_ID,WrongCB_IDs(Index,2)))
                fprintf(fileID,'||(!CB%i.Open &amp;&amp; CB%i.Open &amp;&amp; F%i)',faulted_CB_ID,Upstream_CB_ID,Line(F_branch(1)));
            end
        end
    end
